...
- [ ] `sessions`、`projects`、期間指定の `show`、`search` の `--since` と `--until` が RFC3339 の小数秒をミリ秒へ切り捨てず正確な時点として比較し、保存済みの `2026-03-28T10:00:37.123Z` は `--until 2026-03-28T10:00:37.1235Z` で含まれ、同じ境界の `--since` では除外されるようにする。時点が完全に等しい場合は `--since` を含み `--until` を含まず、等価な UTC オフセットも同一時点として扱い、既存の日付のみ・分単位日時・相対時刻の挙動を維持する。完了には、4 コマンドすべてでこれらを自動検証することを含める。
