...
- [ ] **import 時のメタ前置 INSERT を削除（会話レコードのみが session 行を作る）**
  ...
  - **増分 import を跨ぐ取りこぼし対策は現時点では入れない**（通常運用では発生せず、`--full` で回避可能）。Known limitations に明記する。恒久対応は下記の別タスクで扱う
...
- [ ] **ドキュメント更新**
  ...
  - `internal/core/db.go` の `CREATE TABLE sessions` 内 DDL カラムコメントを `rules/scope.md` の最新文言と揃える（特に `project_dir` の「クエリ側で使わない」注記）

## v0.4 候補

- [ ] **増分 import を跨いだ未紐付けメタの永続バッファ化**（発生は通常運用ではレアで `--full` で回避可能）
  - 問題: `custom-title` / `agent-name` が `user`/`assistant` より先に出現し、メタ行だけが last_offset 内に入った状態で次回 import が走ると、後続の会話追記時に `custom_title` / `agent_name` を反映する手段が無くなる
  - 対応案: (A) 専用テーブルへの永続バッファ、または (B) メタ系のみ毎回先頭再スキャン。実装着手時に方式と詳細コストを評価して決める

- [ ] **`project_dir` カラム撤去可否の再評価**（YAGNI、v0.3 リリース後）
  - v0.3 でクエリ側から `project_dir` 参照を外したため、書き込みは継続するが利用なしの状態が続く。撤去か継続保持かをリリース運用観察後に決める
