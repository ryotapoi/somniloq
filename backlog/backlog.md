...
- [ ] 共通スキーマ型・adapter interface 定義 + 既存 Claude Code ingest の adapter 化（`internal/ingest/` に `Adapter` interface と `NormalizedRecord` 等の共通型を切り、現 `internal/core/import.go` の `processFile` を `internal/ingest/claudecode/` に移して Adapter として再構成。interface は実装と一緒に磨く前提で 1 タスクに統合。挙動は変えない）
