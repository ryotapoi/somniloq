...
- [x] 現在の機能・保存データ・失敗時の保証を保ち、`show` と JSONL 取り込みの不要な構造・状態を整理する
  - `show` の単一・複数セッション表示を共通経路にまとめ、取得 callback、中継関数、表示名の並列配列を削除する
  - 取り込みの未使用なファイル名由来 session ID と `ingest.File` を削除し、パスを直接渡す
  - `ProcessResult.NewOffset` を削除し、差分取り込みの offset は保存された `import_state` を正とする
  - 直接関係する caller・テスト・文書を同期し、出力契約と保存状態の検証を残して共通 gate を通す
