...
- [ ] `cursor-agent` が出力するセッションログの取り込み・保存・検索・表示に対応する
  - 実ログを調査し、取得方法・形式・記録される情報を確認して対応範囲を決める
  - Claude Code / Codex と横断して履歴を参照できるようにし、既存ソースの取り込み・検索が維持されることを検証する
  - 対応範囲に合わせて `docs/rules/`・`docs/specs/`・README を更新する
