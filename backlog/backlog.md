...
- [x] `sessions`、`projects`、`show`、`search` の既存日時フィルタ（`--since`、`--until`、および `sessions --imported-since`）が秒と UTC オフセットを含む RFC3339 値を直接受け付け、明示したオフセットで実行時タイムゾーンに依存しない時点を指定できるようにする。`2026-09-21T09:00:00+09:00` と `2026-09-21T00:00:00Z` は同じ時点として扱い、既存の日付のみ・タイムゾーンなしの分単位日時・相対時刻の入力も引き続き受け付ける。完了には、関連する仕様・help・README の更新と、影響する各コマンドを対象にした自動検証を含める。
- [x] `search <query>` と `sessions`、`show`、`search` の `--project` が、`%`、`_`、およびエスケープ文字自身をワイルドカードではなくリテラルな部分文字列として扱うようにする。`projectAliases` は canonical 名と旧名のどちらを指定しても全名称へ展開し、既存の部分一致、ASCII の大文字小文字を無視する挙動、`/` をまたぐ一致、alias 展開を維持する。正規表現・FTS・性能改善・明示的なワイルドカードモードは対象外とし、実際の需要が出るまで追加しない。完了には、関連する仕様・help・README と互換性に関する案内の更新、`%`、`_`、エスケープ文字自身とそれらの組み合わせ、alias、偽陽性の除外、意図した一致箇所を示す snippet を検証する自動テストを含める。
- [x] `sessions`、`projects`、期間指定の `show`、`search` の `--since` と `--until` が RFC3339 の小数秒をミリ秒へ切り捨てず正確な時点として比較し、保存済みの `2026-03-28T10:00:37.123Z` は `--until 2026-03-28T10:00:37.1235Z` で含まれ、同じ境界の `--since` では除外されるようにする。時点が完全に等しい場合は `--since` を含み `--until` を含まず、等価な UTC オフセットも同一時点として扱い、既存の日付のみ・分単位日時・相対時刻の挙動を維持する。完了には、4 コマンドすべてでこれらを自動検証することを含める。
- [x] `TestProjectAliasLiteralConditionsAcrossCommands` が実行時タイムゾーンに依存せず、UTC−11 / UTC−12 でも `show --since` のalias検証が fixture を誤って除外しないようにする。日付のみの境界をUTCのRFC3339値へ明示するなど、テストの対象であるproject aliasの展開・リテラル一致を変えずに環境依存を除去する。完了にはUTC−11またはUTC−12を固定した自動テストで再現を防ぐことを含める。
