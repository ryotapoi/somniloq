...
- [ ] `TestProjectAliasLiteralConditionsAcrossCommands` が実行時タイムゾーンに依存せず、UTC−11 / UTC−12 でも `show --since` のalias検証が fixture を誤って除外しないようにする。日付のみの境界をUTCのRFC3339値へ明示するなど、テストの対象であるproject aliasの展開・リテラル一致を変えずに環境依存を除去する。完了にはUTC−11またはUTC−12を固定した自動テストで再現を防ぐことを含める。
