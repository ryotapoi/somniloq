...
- [ ] sidechain 除外フィルタを 1 層に寄せる: `GetSummaryMessages` は SQL（`is_sidechain = 0`）、show 全文表示は表示層（`cmd/somniloq/format.go` の `formatSession` 内 skip）と、同じ仕様が 2 層に分散している。SQL 側に寄せる
