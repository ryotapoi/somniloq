...
- TEXT のバイト数は `OCTET_LENGTH(text)` で取る。`LENGTH(text)` は文字数を返す。`internal/core/db_sessions_projects.go` の `sessionRowSelect` は `show` が出す本文量に合わせるため、sidechain を除外して `OCTET_LENGTH(m.content)` を集計する。
