...
| `backfill` | `cmd/somniloq/backfill.go` | `internal/core/backfill.go` | `cmd/somniloq/backfill_test.go`, `internal/core/backfill_test.go` | `docs/rules/scope.md` の バックフィル |
| `sessions` | `cmd/somniloq/sessions.go` | `internal/core/db.go` の `ListSessions` | `cmd/somniloq/sessions_test.go`, `internal/core/db_query_test.go` | `docs/rules/scope.md` の セッション一覧 |
...
- time / project filter を変える: `cmd/somniloq/filter.go` -> `internal/core/db.go` の `timeFilterConditions` / `projectsCondition` -> query tests。
