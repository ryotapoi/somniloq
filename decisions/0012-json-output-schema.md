...
- `--short` は JSON の `project` フィールドにも反映する。「`--short` は常に basename」という既存契約に出力形式による例外を作らない。生パスが欲しい消費側は `--short` を付けなければよい
...
- `--short` 付き JSON では生 `repo_path` が取れない。実害が出たら raw フィールド追加を別途検討する
