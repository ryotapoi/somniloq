...
| コマンド | 説明 |
|---------|------|
| `import` | Claude Code の JSONL ファイルを SQLite に取り込む |
| `import-codex` | Codex の JSONL ファイルを SQLite に取り込む |
...
Claude Code の JSONL を `~/.claude/projects/` から取り込む。

### import-codex

```bash
somniloq import-codex              # 差分取り込み
somniloq import-codex --full       # 全件再取り込み（確認プロンプトあり）
somniloq import-codex --full --yes # 確認なしで全件再取り込み
```

Codex の rollout JSONL を `~/.codex/sessions/` から取り込む。
...
- `import` は Claude Code 専用のまま。Codex の rollout log は `import-codex` を使う。
