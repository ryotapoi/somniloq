...
- **Acceptance**:
  ...
  - Goal 開始時 base 以降の commit 済み内容が Claude review 済みで、対応必須の指摘が残っていない。
...
- レビューループ（commit 内のレビュー周回、Claude review 実行回数）が上限の目安を超過した場合は、Goal 完了時に `レビュー上限超過` として対象単位・回数・要因となった指摘・収束結果を提示する。超過がない場合も `レビュー上限超過: なし` と明示する。
...
- 各レビュー単位につき Claude review 実行は 3 回を目安とする。超過しても止まらず収束まで続け、超過の事実（実行回数・要因となった指摘・収束結果）を記録して Goal 完了報告の `レビュー上限超過` で通知する。
...
- Claude review の対応必須の指摘が、周回を重ねても解消も却下もできず収束しない。
