...
- **`SUMMARY: ... needs_action=YES ...`（⛔ 分割推奨）**: `RESULT_FILE` のパスを Read で読み込み（`/tmp/claude/claude-review-results/` 配下であることを確認）、検出シグナルをユーザーに提示し、`AskUserQuestion` で以下を尋ねる:
  ...
  選択肢 1 の場合、以降のレビュー（手順 1 以降）は**全てスキップ**し、`rules/workflow/1c-plan.md`（または該当する workflow ファイル）の「プラン段階で分割が必要と判明した場合の手順」に従って ExitPlanMode をユーザーに依頼する旨を伝えて終了する。選択肢 2 の場合は次の手順 1 に進む（プラン本文に「分割しないと判断した理由」を追記してから）。
...
- 2 回目以降:
  - 前回 Codex レビューの結果ファイルを Read で読む。**ファイルパスは前回 `/review-plan-codex` の戻り値テキストから抽出した `RESULT_FILE:` の値をそのまま使う**（main の会話履歴に残っている）。タイムスタンプや glob で「直近のもの」を選ぶのは禁止 — 他セッションが並行稼働している時に他人のファイルを掴む。`/tmp/claude/claude-review-results/` 配下であることを確認する。Read は main context で 1 回だけ行う（fork 内では結果ファイルが見えない）
  - 各指摘について「対応した（→ 修正内容を 1 行）」または「対応しない（→ 理由を 1 行、プランの「考慮したこと」と整合）」を main で確定する（収束条件は `rules/workflow/1c-plan.md` の「レビューの収束条件」）
  - args 文字列を以下の形式で組み立てて渡す。Claude 側の見解（「これは本質的じゃない」等）は混ぜず、事実（何を変えたか・どの指摘を残したか）だけ伝える:
    ...
    - 🟡 <指摘2の要旨> → ...
    新規問題と、対応の妥当性を確認してください。
    ```

  Codex 自身が前回ログを覚えているので、指摘の全文を貼り直す必要はない。**要旨 1 行 + 対応 1 行**で十分。
