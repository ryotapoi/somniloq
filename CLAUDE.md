...
- **計画フェーズ着手時**: rules/workflow/1a-ux-scenario.md / 1b-design.md / 1c-plan.md をまとめて Read してから 1a に入る。2a 以降は実装フェーズに入った時点で読む（計画と実装はセッションを分断するため）
...
- **不明点があれば止まって確認する。** 不明点はユーザーに確認してから進める
- **不明点がなければ workflow を続ける。** 自動進行がデフォルト。ワークフロー詳細に明示的な停止指示がある場合のみ止まる
- **コミットまで終えたら止まる。** 次のタスクはユーザーの指示を待ってから着手する
...
コミットは workflow 内外を問わず、必ず `/commit` スキル経由で行う。
