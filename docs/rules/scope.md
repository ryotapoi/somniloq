...
- 出力 TSV の列: `session_id`, `started_at ~ ended_at`, `project`, `custom_title`, `message_count`, `body_size`, `non_command_user_turn_count`, `first_non_command_user_line`
...
- `--format tsv|json`（デフォルト `tsv`）。JSON のフィールドは `source`, `sessionId`, `project`, `title`, `startedAt`, `endedAt`, `messageCount`, `bodySize`, `nonCommandUserTurnCount`, `firstNonCommandUserLine`（共通仕様は「JSON 出力」節参照）
